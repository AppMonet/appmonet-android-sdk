...
//    val adViewManager = spyk(adViewManagerComponent.adViewManager)
  ...
  @Test fun `Given hasPermission is called and permission is not granted return false`() {
    val adViewManagerComponent = adViewManagerSetup()
    val adViewManager = spyk(adViewManagerComponent.adViewManager)
    mockkStatic(RenderingUtils::class)
    every { RenderingUtils.getFlagByName(any(), any()) } answers { "" as Object }
    assertEquals(adViewManager.hasPermission(""), false)
  }

  @Test
  fun `Given hasPermission is called and permission is  granted return ContextCompat checkSelfPermission`() {
    val adViewManagerComponent = adViewManagerSetup()
    val adViewManager = spyk(adViewManagerComponent.adViewManager)
    mockkStatic(RenderingUtils::class)
    mockkStatic(ContextCompat::class)
    every { ContextCompat.checkSelfPermission(any(), any()) } answers { 0 }
    every { RenderingUtils.getFlagByName(any(), any()) } answers { "not_empty" as Object }
    assertEquals(adViewManager.hasPermission(""), true)
  ...
  @Test
  fun `Given isScreenOn RenderUtils isScreenOn is called`() {
    val adViewManagerComponent = adViewManagerSetup()
    mockkStatic(RenderingUtils::class)
    adViewManagerComponent.adViewManager.isScreenOn()
    verify(exactly = 1) { RenderingUtils.isScreenOn(any()) }
  }

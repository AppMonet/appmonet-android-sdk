...
class AdViewManager : AdViewManagerCallback {
  ...
  override fun hasPermission(permission: String): Boolean {
    val flag = try {
      RenderingUtils.getFlagByName(permission::class.java, permission) as String
    } catch (e: Exception) {
      ""
    }
    if (flag.isEmpty()) {
    ...
    return ContextCompat
        .checkSelfPermission(context.applicationContext, flag) == PackageManager.PERMISSION_GRANTED
  }
  ...
  override fun isScreenOn(): Boolean {
    return RenderingUtils.isScreenOn(context)
  }

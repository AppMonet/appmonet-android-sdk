...
import com.monet.ICustomEventInterstitialAdapter
import com.monet.MediationCallback
import com.monet.RenderingInterstitialAdapterResponse
...
//  private var bidResponse: BidResponse? = null
  ...
  private var customEventInterstitialAdapter: ICustomEventInterstitialAdapter? = null
  ...
  override fun requestInterstitialAd(
    ...
    mAdServerListener = MonetDfpInterstitialListener(listener, context)
    customEventInterstitialAdapter = CustomEventInterstitialAdapter(
        customEventExtras?.toMap(),
        serverParameter, adSize, mAdServerListener, sdkManager, sdkManager?.auctionManager,
        sdkManager?.auctionManager?.bidManager, sdkManager?.auctionManager?.mediationManager
    ) { bid: BidResponse? ->
      try {
        ...
        if (bid!!.interstitial != null && bid!!.interstitial!!.trusted) {
          mAdServerListener?.onAdLoaded(null)
          RenderingInterstitialAdapterResponse(null, true)
        }

        RenderingInterstitialAdapterResponse(
            BidRenderer.renderBid(context, sdkManager!!, bid, null, mAdServerListener!!)
        )
      } catch (e: Exception) {
        RenderingInterstitialAdapterResponse(null)
      }
    }
...
//  }
  ...
  override fun showInterstitial() {
    val adView = customEventInterstitialAdapter?.adView
    val bidResponse = customEventInterstitialAdapter?.currentBid
    ...
    sdkManager!!.preferences.setPreference(AD_UUID_INTERSTITIAL, adView?.uuid)
    MonetDfpActivity.start(mContext!!, sdkManager!!, adView?.uuid, bidResponse?.adm)
  }
